...
void* calculate_jacobi_fun(void* _args)
{
	...
	struct jacobi_fun_args *args = (struct jacobi_fun_args*)_args;
	
	/* over all rows */
	for (i = args->first_i; i <= args->last_i; i++)
	{
		double fpisin_i = 0.0;
		if (args->opts->inf_func == FUNC_FPISIN)
		{
		...
		for (j = 1; j < args->N; j++)
		{
			...
			if (args->opts->termination == TERM_PREC || args->term_iteration == 1)
			{
				residuum = args->Matrix_In[i][j] - star;
				residuum = (residuum < 0) ? -residuum : residuum;
				if (residuum > *(args->maxresiduum)) {
					assert(!pthread_mutex_lock(args->residuummutex));
					*(args->maxresiduum) = residuum;
					assert(!pthread_mutex_unlock(args->residuummutex));
				}
			}
...
static void calculate_jacobi (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options)
{
	...
	while (term_iteration > 0)
	{
		...
		for(unsigned int k = 0; k < options->number; k++){
			...
			assert(!pthread_create(&threads[k], NULL, &calculate_jacobi_fun, &args[k]));
		}
...
static void calculate_gauss_seidel (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options)
{
